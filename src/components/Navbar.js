import React from "react";
import Navbar from "react-bootstrap/Navbar";
import Nav from "react-bootstrap/Nav";

window.onscroll = function(ev) {
  if ((window.pageYOffset) >= document.getElementById('blogsDiv').offsetTop - document.getElementById('navbarDiv').clientHeight) {
    document.getElementById('navbarDiv').style.backgroundColor = '#3498db';
  }
  else if ((window.pageYOffset) >= document.getElementById('categoriesDiv').offsetTop - document.getElementById('navbarDiv').clientHeight) {
    document.getElementById('navbarDiv').style.backgroundColor = '#2c3e50';
  }
  else if ((window.pageYOffset) >= document.getElementById('servicesDiv').offsetTop - document.getElementById('navbarDiv').clientHeight) {
    document.getElementById('navbarDiv').style.backgroundColor = '#3498db';
  }
  else {
    document.getElementById('navbarDiv').style.backgroundColor = '#2c3e50';
  }
};

function Navigator(props) {
  return (
    <Navbar id="navbarDiv" collapseOnSelect expand="lg" bg="" variant="dark" fixed='top' style={{boxShadow: '0 -10px 30px ghostwhite', backgroundColor: "#2c3e50"}}>
      <Navbar.Brand href="https://disingenuous-perfor.000webhostapp.com">
        <img
          style={{ maxWidth: "130px" }}
          src="data:image/png;base64,"
          alt="logo"
        ></img>
      </Navbar.Brand>
      <Navbar.Toggle aria-controls="responsive-navbar-nav" />
      <Navbar.Collapse id="responsive-navbar-nav">
        <Nav className="m-auto">
          <Nav.Link className="NavbarCustomElement mx-2" href="https://disingenuous-perfor.000webhostapp.com">
            <i className="fa fa-home" ></i> Home
          </Nav.Link>
          <Nav.Link className="NavbarCustomElement mx-2" href="https://disingenuous-perfor.000webhostapp.com">
            <i className="fa fa-shopping-basket"></i> Shop
          </Nav.Link>
          <Nav.Link className="NavbarCustomElement mx-2" href="https://disingenuous-perfor.000webhostapp.com">
            <i className="fa fa-newspaper-o"></i> Blog
          </Nav.Link>
          <Nav.Link className="NavbarCustomElement mx-2" href="https://disingenuous-perfor.000webhostapp.com">
            <i className="fa fa-address-card"></i> Contact Us
          </Nav.Link>
        </Nav>
        <Nav>
          <Nav.Link href="https://disingenuous-perfor.000webhostapp.com">
            <i className="fa fa-cart-arrow-down" style={{ fontSize: "22px" }} />{" "}
            Cart{" "}
            <span className="badge badge-pill badge-danger">
              {props.TotalItems}
            </span>
          </Nav.Link>
          <Nav.Link href="https://disingenuous-perfor.000webhostapp.com">
            <i className="fa fa-user-circle-o" style={{ fontSize: "22px" }} />
            {props.UserName}{" "}
          </Nav.Link>
        </Nav>
      </Navbar.Collapse>
    </Navbar>
  );
}
export default Navigator;
